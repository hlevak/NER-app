...
└─────────────────────────────────────────────────────────────┘
              ...
              │               │               │
    ┌─────────▼────────┐ ┌────▼────┐ ┌────────▼────────┐
    │  Внешний WebAPI  │ │LM Studio│ │ llama.cpp       │
    │  (поиск сущн.)   │ │(local)  │ │ (local server)  │
    └──────────────────┘ └─────────┘ └─────────────────┘
```
...
- Возвращает автоматическую NER-разметку (ансамбль spaCy + WebAPI + LLM)
- Обучает/дообучает модель через `/fit`
- Предоставляет автоподсказки через `/suggestions`
- Нормализует сущности через `/normalize`

**Конфигурация:** `config/ml-backend.env`

**Приоритет источников:** LLM → WebAPI → spaCy
...
3. ML Backend:
   ...
   c. (опционально) LLM предсказание → список сущностей
   d. Сущности объединяются (приоритет: LLM > WebAPI > spaCy)
...
│   ├── predict_entities()    # LLM NER предсказание
│   ├── normalize_entity()    # Нормализация через LLM
│   └── get_suggestions()     # Автоподсказки
│
...
LLM results:    [PER(0,5), MONEY(50,60)]

Merge (приоритет: LLM > WebAPI > spaCy):
  1. Объединяем WebAPI + spaCy:
     - Берём все WebAPI сущности: [PER(0,6), DATE(35,42)]
     - Добавляем spaCy без пересечений: [LOC(15,21), ORG(25,30)]
     - Результат: [PER(0,6), LOC(15,21), ORG(25,30), DATE(35,42)]
  2. Объединяем LLM + результат из шага 1:
     - Берём все LLM сущности: [PER(0,5), MONEY(50,60)]
     - PER(0,5) пересекается с PER(0,6) из WebAPI → заменяем
     - MONEY(50,60) не пересекается → добавляем
  3. Финальный результат: [PER(0,5), LOC(15,21), ORG(25,30), DATE(35,42), MONEY(50,60)]
                           ↑ из LLM   ↑ из spaCy   ↑ из spaCy  ↑ из WebAPI   ↑ из LLM
```
...
├── llm.log          # LLM операции
├── llm_errors.log   # Ошибки LLM (ERROR+)
